...
export enum ChartType {
  WON = 'Parties gagnées',
  PLAYED_AND_WON = 'Parties jouées et gagnées',
  RATIOS = 'Ratios',
}
...
export class TrioComponent implements OnInit {
  ...
  private setWonChar(): void {
    ...
    this.data = {
      labels: this.ratios
        .filter((ratio) => ratio.wins > 0)
        .map((ratio: TrioRatio) => ratio.playerName),
      datasets: [
        {
          data: this.ratios
            .filter((ratio) => ratio.wins > 0)
            .map((ratio) => ratio.wins),
        },
      ],
    };
    ...
    this.data = {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: this.ratios.map(
            (value: TrioRatio) => value.gamesPlayed - value.wins,
          ),
        },
      ],
    };
